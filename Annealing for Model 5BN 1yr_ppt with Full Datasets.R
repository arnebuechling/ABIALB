...
if (index == 4) { load(paste(spp_list[index],"Model 5BN Full Azure Results 2.Rdata")) } else
                { load(paste(spp_list[index],"Model 5BN Full Azure Results 2.Rdata")) }
...
if (index == 4) { par <- model_5BN_full_azure_results_2$best_pars } else
                { par <- model_5BN_full_azure_results_2$best_pars  }

if (index == 1) { nspp <- 7 } else
                { nspp <- 5 } 
par$lambda <- par$lambda[1:nspp]

par <- par[-c(24,26,28,30,32)]
...
model_5BN_1yr_ppt_full_azure_results <- anneal(model=model_5BN_1yr_ppt, par=par, var=var, source_data=working, par_lo=par_lo, par_hi=par_hi,
			  pdf=linear_dnorm_with_intercept, dep_var="incr_mm", max_iter=iterations)

setwd(out_dir)

write_results(model_5BN_1yr_ppt_full_azure_results, file = paste(spp_list[index],"Model 5BN 1yr_ppt Full Azure Results.txt", sep=" "), data=F, print_whole_hist=F)
save(model_5BN_1yr_ppt_full_azure_results, file=paste(spp_list[index],"Model 5BN 1yr_ppt Full Azure Results.Rdata",sep=" "))
