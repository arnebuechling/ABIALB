...
model_5BN_full_azure_results2 <- anneal(model=model_5BN, par=par, var=var, source_data=working, par_lo=par_lo, par_hi=par_hi,
			  pdf=linear_dnorm_with_intercept, dep_var="incr_mm", max_iter=iterations)

setwd(out_dir)

write_results(model_5BN_full_azure_results2, file = paste(spp_list[index],"Model 5BN Full Azure Results 2.txt", sep=" "), data=F, print_whole_hist=F)
save(model_5BN_full_azure_results2, file=paste(spp_list[index],"Model 5BN Full Azure Results 2.Rdata",sep=" "))
